    ...
    protected static String result(String input) {
        ...
        result =
                IntStream.range(0, input.length() / 2)
                        .filter(i -> i % 2 == 0)
                        .mapToObj(i -> {
                            int firstDigit = Character.getNumericValue(input.charAt(i * 2)) * 10;
                            int secondDigit = Character.getNumericValue(input.charAt(i * 2 + 1));
                            return (char) (firstDigit + secondDigit);
                        })
                        .map(String::valueOf)
